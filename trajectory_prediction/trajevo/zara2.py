...
def predict_trajectory(trajectory: np.ndarray) -> np.ndarray:
    ...
    for _ in range(future_len):
        ...
        direction_to_goal_norm = direction_to_goal / (np.linalg.norm(direction_to_goal, axis=1, keepdims=True) + 1e-6)
        current_vel_norm = current_vel / (np.linalg.norm(current_vel, axis=1, keepdims=True) + 1e-6)
        new_direction = (1 - turn_strength) * current_vel_norm + turn_strength * direction_to_goal_norm
    ...
    smoothed_traj = trajectory[:, -1:, :] + time_steps * np.expand_dims(smoothed_velocity, axis=1)
    ...
    jerky_accel = ((trajectory[:, -1, :] - trajectory[:, -2, :]) - (trajectory[:, -2, :] - trajectory[:, -3, :]))[:, np.newaxis, :]
    kinematic_traj = initial_pos + initial_vel * time_steps + 0.5 * jerky_accel * (time_steps ** 2)
    ...
    smoothed_traj = trajectory[:, -1:, :] + time_steps * np.expand_dims(corrected_velocity, axis=1)
    all_trajectories.append(smoothed_traj)
    ...
    circumvent_trajectory = trajectory[:, -1:, :] + time_steps * np.expand_dims(rotated_velocity, axis=1)
