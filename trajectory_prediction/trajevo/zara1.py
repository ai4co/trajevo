...
def predict_trajectory(trajectory: np.ndarray) -> np.ndarray:
    ...
    for i in range(20):
        ...
        if i < 14:  # Increased to 14, best performing strategy
            ...
            noise_scale = 0.012 + current_avg_speed * 0.007 # Slightly reduced avg_speed influence on noise
            ...
            if i % 6 == 0:
                ...
                momentum = np.random.uniform(0.07, 0.15)  # Slightly adjusted momentum range
                ...
                noise_scale = 0.01 + current_avg_speed * np.random.uniform(0.007, 0.015) # Adjusted adaptive noise
                noise = np.random.normal(0, noise_scale, size=(num_agents, 12, 2))
        ...
        # Option 2: Velocity rotation with adaptive angle
            ...
            current_avg_speed = np.mean(np.linalg.norm(velocity, axis=1)) # Using current velocity for avg_speed
            angle_scale = 0.12 + current_avg_speed * 0.045  # adaptive angle, slightly reduced influence
            ...
            noise_scale = 0.006 + current_avg_speed * 0.0035 # Slightly reduced noise scale
            noise = np.random.normal(0, noise_scale, size=(num_agents, 12, 2))
        ...
        # Option 3: Memory-based approach (repeating last velocity) + Enhanced Collision Avoidance
            ...
            if history_len > 3:
                velocity = (
                    ...
                    + 0.35 * (trajectory[:, -2, :] - trajectory[:, -3, :]) # Increased weight for older velocity
            ...
            noise_scale = 0.004 + current_avg_speed * 0.001 # Slightly reduced noise scale
            noise = np.random.laplace(0, noise_scale, size=(num_agents, 2))
            velocity = velocity + noise

            # Enhanced collision avoidance
            repulsion_strength = 0.0013  # Slightly increased repulsion strength for clearer separation
            predictions = []
            ...
            for t in range(1, 13):
                        ...
                        if agent_idx != other_idx:
                            ...
                            if distance < 1.1:
                                repulsion = (
                                    ...
                                    * repulsion_strength
                                    * np.exp(-distance * 2) # Stronger exponential decay for closer proximity
                                )
                ...
                velocity = (0.85 * velocity + 0.15 * net_repulsions) # More influence from repulsion
        ...
        # Option 4: Linear prediction with adaptive damping and larger noise.
            ...
            velocity = trajectory[:, -1, :] - trajectory[:, -2, :]
            damping = np.random.uniform(0.015, 0.035)  # Slightly adjusted damping factor range
