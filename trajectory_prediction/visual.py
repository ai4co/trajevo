...
def visualize_trajectory(data, predictions, num_alternatives=10):
    """
    ...
    for i in range(num_agents):
        ...
        for alt_idx in alternative_indices:
            plt.plot(
                predictions[alt_idx, i, :, 0],
                predictions[alt_idx, i, :, 1],
                color=color,
                linestyle=":",
                alpha=0.3,
                linewidth=1,
    ...
    plt.legend(handles=legend_elements, loc="upper left", title="Agent")
    ...
    plt.title(f"Trajectory Prediction (TimeStep: {time_step}, {num_agents} Agents)")
...
def process_multiple_scenarios(trajectories, frame_ids, scenarios, num_alternatives=5):
    """
    ...
    for i, (frame, agents) in enumerate(scenarios):
        ...
        visualize_trajectory(data, predictions, num_alternatives)


def main():
    data_file = "./datasets/eth/test/biwi_eth.txt"

    trajectories, frame_ids, agent_ids = load_data(data_file)

    scenarios = find_multiple_valid_scenarios(trajectories, frame_ids, num_scenarios=15)

    if scenarios:
        # set the number of alternative predictions to display (besides the best one)
        num_alternatives = 5
        process_multiple_scenarios(trajectories, frame_ids, scenarios, num_alternatives)
    else:
        print("can't find any valid scenarios")
